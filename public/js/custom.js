...
function getColumnPrefKey(tableId) {
    var path = window.location.pathname.replace(/\W+/g, '_');
    return 'erpgo_table_cols_' + tableId + '_' + path;
}

function readColumnPrefs(key) {
    try {
...
function saveColumnPrefs(dataTable, tableId) {
    var hidden = [];
    var cols = dataTable.columns().visible();
    for (var i = 0; i < cols.length; i++) {
        if (!cols[i]) hidden.push(i);
    }
    try {
        localStorage.setItem(getColumnPrefKey(tableId), JSON.stringify(hidden));
    } catch (e) {}
}

function applyColumnPrefs(dataTable, tableId) {
    var hidden = readColumnPrefs(getColumnPrefKey(tableId));
    if (hidden.length) {
        dataTable.columns().hide(hidden);
    }
}

function buildColumnToggles(dataTable, tableId) {
    var wrapper = dataTable.wrapper;
    if (!wrapper) return;
    var top = wrapper.querySelector('.dataTable-top');
    if (!top) return;
    if (top.querySelector('[data-columns-for="' + tableId + '"]')) return;

    var dropdown = document.createElement('div');
    dropdown.className = 'dropdown datatable-columns-dropdown';
    dropdown.setAttribute('data-columns-for', tableId);
    dropdown.innerHTML = '<button class="btn btn-light btn-sm dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">Columns</button><div class="dropdown-menu p-2"></div>';
    var menu = dropdown.querySelector('.dropdown-menu');

    dataTable.headings.forEach(function (th, idx) {
        var label = document.createElement('label');
        label.className = 'dropdown-item d-flex align-items-center gap-2';
        var checkbox = document.createElement('input');
        checkbox.type = 'checkbox';
        checkbox.className = 'form-check-input m-0';
        checkbox.checked = dataTable.columns().visible(idx);
        checkbox.addEventListener('change', function () {
            var visible = dataTable.columns().visible();
            var visibleCount = 0;
            ...
            if (checkbox.checked) {
                dataTable.columns().show([idx]);
            } else {
                dataTable.columns().hide([idx]);
            }
            saveColumnPrefs(dataTable, tableId);
        });
...
$(document).ready(function () {
    if ($(".datatable").length > 0) {
        document.querySelectorAll('.datatable').forEach(function (table, index) {
            var tableId = ensureTableId(table, index);
            var dataTable = new simpleDatatables.DataTable(table);
            applyColumnPrefs(dataTable, tableId);
            buildColumnToggles(dataTable, tableId);
        });
    }
...
$(document).ready(function () {
    ...
    if ($(".pc-dt-simple").length > 0) {
        $( $(".pc-dt-simple") ).each(function( index,element ) {
            ...
            if (!id) {
                id = 'pc-dt-simple-' + index + '-' + Date.now();
                $(element).attr('id', id);
            }
            const dataTable = new simpleDatatables.DataTable("#"+id);
            applyColumnPrefs(dataTable, id);
            buildColumnToggles(dataTable, id);
        });
    }
